    ...
    internal class NetworkFunctionRegistry
    {
        ...
        public virtual void RegisterAllGameObjects()
        {
            ...
            if (registeredObjects.Distinct().Count() != registeredObjects.Count)
            {
                ...
                var message =
                    $@"The scene contains multiple networked game objects with the same name: {duplicateNames}. 
        ...
        public virtual bool RegisterGameObject(GameObject target, bool doRegisterChildObjects)
        {
            ...
            if (!doRegisterChildObjects)
                return wasRegistered;
            return target.ChildGameObjects().Aggregate(false, (current, x) => RegisterGameObject(x) | current) | wasRegistered;
        }

        private bool RegisterGameObject(GameObject target) => target.ThrowIfNull().MonoBehaviours().Aggregate(false, (current, x) => RegisterObject(x) | current);
        ...
        public virtual bool RegisterObject(object target)
        {
            ...
            {
                var message =
                    $@"The object {target} is already registered. 
        ...
        private bool RegisterObjectWithType(object target, ulong objectId, Type type)
        {
            ...
            foreach (var data in functionData)
                RegisterObjectFunction(target, objectId, data);
            return true;
        }
        ...
        private void RegisterObjectFunction(object target, ulong objectId, NetworkFunctionData functionData)
        {
            ...
            {
                var message =
                    $@"The object {target} contains multiple instances of the {functionData.MethodInfo.Name} network function.
        ...
        {
            if (FunctionCache.TryGetValue(type, out var functions))
                return functions;
        ...
        private NetworkFunctionData[] FilteredNetworkFunctionData(IEnumerable<MethodInfo> methodInfo) =>
            methodInfo
                .Select(x => (x, (NetworkFunction)x.GetCustomAttribute(typeof(NetworkFunction))))
                .Where(pair => pair.Item2 != null)
                .Select(pair => new NetworkFunctionData(pair.Item2, pair.Item1))
                .ToArray();
        ...
        public virtual ulong GetRegisteredObjectId(object target)
        {
            ...
            {
                var message =
                    $@"The requested object {target} is not registered. 
        ...
        public virtual object GetRegisteredObject(ulong objectId)
        {
            ...
            {
                if (objectId < RegistrationCounter.Value)
                    return null;

                var message =
                    $@"The requested object id {objectId} is not registered. 
                    Make sure to register all objects before calling network function on them or passing them as arguments.
                    Use the registration extension functions to register objects.";
        ...
        public virtual NetworkFunctionDelegate GetRegisteredFunctionDelegate(ulong objectId, NetworkHash functionId)
        {
            if (!ObjectFunctions.Contains(objectId))
            {
                ...
                var message =
                    $@"There are no function registered for the requested object id {objectId}. 
                    Make sure to register all objects before calling network function on them or passing them as arguments.
                    Use the registration extension functions to register objects.";
            ...
            {
                var message =
                    $@"The requested function with hash {functionId.GetHashCode()} was not found for the given object {objectId}. 
