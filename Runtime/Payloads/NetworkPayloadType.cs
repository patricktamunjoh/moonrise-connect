...
namespace MoonriseGames.CloudsAhoyConnect.Payloads {
    ...
    internal class NetworkPayloadType {
        ...
        public INetworkPayload CreateInstanceFromArguments(NetworkFunctionRegistry registry, params object[] arguments) {
            ...
            var replacedArguments = arguments.Zip(ObjectReplacementPositions, (parameter, isReplaced) => (parameter, isReplaced))
        ...
        public object[] RecoverArgumentsFromInstance(NetworkFunctionRegistry registry, INetworkPayload payload) => payload?.Arguments()
            .Take(ArgumentCount).Zip(ObjectReplacementPositions, (arg, isReplaced) => (arg, isReplaced))
            .Select(x => x.isReplaced ? registry.GetRegisteredObject((ulong)x.arg) : x.arg).ToArray();
        ...
        private Type RemotablePayloadType(IReadOnlyCollection<Type> parameterTypes, out bool[] replacedPositions) {
            if (parameterTypes.Count > ParameterCount) {
                var message = $@"The number of supplied type parameters exceeds the maximum value of {ParameterCount}.
        ...
        private Type RemotableParameterType(Type type, out bool isReplaced) {
            ...
            if (!isReplaced && !IsSerializable(type)) {
                var message = $@"Objects of type {type} cannot be used as arguments to network functions.
        ...
        private bool IsSerializable(Type type) {
            if (type.IsDefined(typeof(SerializableAttribute))) return true;
            return IsPrimitiveType(type) || IsSerializableBuiltInType(type) || IsSerializableCollectionType(type);
        }

        private bool IsPrimitiveType(Type type) {
            if (type.IsPrimitive || type.IsEnum || type == typeof(decimal) || type == typeof(string)) return true;
            return false;
        }

        private bool IsSerializableBuiltInType(Type type) {
            if (type == typeof(Vector2) || type == typeof(Vector3) || type == typeof(Vector4) || type == typeof(Quaternion)) return true;
            if (type == typeof(Vector2Int) || type == typeof(Vector3Int)) return true;
            if (type == typeof(Matrix4x4) || type == typeof(Color) || type == typeof(Rect) || type == typeof(LayerMask)) return true;
            return false;
        }

        private bool IsSerializableCollectionType(Type type) {
            if (type.IsArray && IsSerializable(type.GetElementType())) return true;
            if (type == typeof(List<>) && IsSerializable(type.GetGenericArguments().Single())) return true;
            return false;
        }

        private IEnumerable<T> FillMissingValues<T>(IEnumerable<T> types, T value, int size) =>
            types.Concat(Enumerable.Repeat(value, size));
