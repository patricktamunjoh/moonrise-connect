...
namespace MoonriseGames.CloudsAhoyConnect.Logging {
    ...
    public class Snapshot {
        ...
        /// </summary>
        public ReadOnlyCollection<(string recipient, ulong id, string function, Transmission transmission)> OutgoingNetworkCalls =>
            OutgoingCalls.AsReadOnly();
        ...
        /// target function and the transmission type.
        /// </summary>
        public ReadOnlyCollection<(string sender, ulong id, string function, Transmission transmission)> IncomingNetworkCalls =>
            IncomingCalls.AsReadOnly();
        ...
        /// the events raised by <see cref="CloudsAhoyConnect.OnNetworkConnectionChanged" />
        /// </summary>
        public ReadOnlyCollection<(NetworkConnectionEventArgs.Types type, string target)> NetworkEvents => Events.AsReadOnly();

        internal virtual void RecordObjectRegistration(ulong id, object target) =>
            Registrations.Add((id, target.ThrowIfNull().GetType(), target.ThrowIfNull().ToString()));

        internal virtual void RecordObjectUnregistration(ulong? id, object target, string info = null) =>
            Unregistrations.Add((id, target?.GetType(), target?.ToString(), info));
